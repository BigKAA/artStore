"""
Асинхронный Redis клиент для Ingester Module.

Sprint 14: Redis Storage Registry & Adaptive Capacity.

Используется для:
- Получения списка доступных Storage Elements из Redis Registry
- Service Discovery для Storage Elements
- Подписки на обновления конфигурации SE
...
class RedisCircuitBreaker:
    ...
    - open: Redis недоступен, запросы блокируются
    - half-open: Пробный период, ограниченное количество запросов

    После failure_threshold неудачных запросов circuit breaker открывается.
    После recovery_timeout секунд переходит в half-open состояние.
    При успешном запросе в half-open возвращается в closed состояние.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            recovery_timeout: Время в секундах до перехода в half-open
            half_open_max_calls: Макс. запросов в half-open состоянии
        """
    ...
    def state(self) -> str:
        ...
        # Обновляем состояние при проверке
        _ = self.is_open
        return self._state
