...
Схемы для events, получаемых от Admin Module через Redis Pub/Sub.
...
class FileMetadataEvent(BaseModel):
    """
    Метаданные файла из event payload.
...
class FileCreatedEvent(BaseModel):
    """
    Event получаемый после успешной регистрации файла в Admin Module.

    Источник: Admin Module EventPublisher.
    Обработка: Query Module EventSubscriber → CacheSyncService.
    """
...
class FileUpdatedEvent(BaseModel):
    """
    Event получаемый после обновления файла в Admin Module.
...
class FileDeletedEvent(BaseModel):
    """
    Event получаемый после удаления файла в Admin Module (soft delete).
